...
5. **IP 축 에지 차단 ≠ 세션 축 인가.** XDP 드랍은 세션 무효화(주체 단위, D1)의 대체가 아니라
...
```text
  토큰 없는 SYN 플러드 (172.18.0.1)
        │
        ▼  (게이트웨이 컨테이너 netns eth0, ingress)
  ┌───────────────── XDP (rate_enforce.c) ─────────────────┐
  │  ① 관측: per-source-IP {pkts, syns} 누적 (LRU HASH)     │
  │  ② 집행: deny map에 있고 TTL 안이면 → XDP_DROP            │  ← 스택 진입 전
  └───────────────┬───────────────────────────▲────────────┘
      src_ip_stats│ (map 폴링 1s)              │ deny map 기록
                  ▼                           │
        ┌──────────────────── Go 에이전트 ──────┴──────┐
        │  윈도우(5s) SYN 차분 → 임계(20) 초과 판정       │
        │  PIP에 rate-l4 신호(mTLS POST)                │
        │  PIP ack의 enforcement 지시를 deny map에 번역  │
        │  만료 엔트리 sweep(위생)                       │
        └────────────────┬──────────────────▲──────────┘
             rate-l4 신호 │                  │ ack{reassessed, enforcement:{deny,ttl}}
                         ▼                  │
              ┌──────────────── PIP (판단 주체) ───────────┐
              │  IP→주체 번역(lastSeenIp) → 재평가(D1 경로): │
              │    rate-l4 +40 → epoch bump → 능동 무효화   │
              │  + 에지 차단 지시(enforcement) ack로 반환    │
              └───────────────────────────────────────────┘
```
...
핵심은 **D3가 새 enforcement 체계가 아니라 D1 파이프의 새 입력**이라는 것이다. L4 신호는 요청
밖(out-of-band)에서 IP 단위로 도착하는데 인가는 주체 단위다 → PIP가 `lastSeenIp`로 IP를 주체로
번역한 뒤 기존 `assess`를 태우면, 점수변화→epoch→능동 무효화가 그대로 발화한다. XDP 드랍은 거기에
"에지 축"을 하나 더 얹은 것이다.
...
- 에이전트가 죽어 엔트리를 못 지워도, TTL이 지나면 커널이 그냥 통과시킨다 → **오탐이 영구 차단으로
  굳지 않는다**. 차단의 지속은 "살아 있는 판단 주체(PIP→에이전트)의 갱신"에만 의존한다(D1 kill-switch
  가역성과 같은 논리).
...
프로그램의 상시 오버헤드는 양팔에 공통이고 순수하게 "드랍의 효과"만 남는다 — Phase 5의 "게이트웨이만
...
> 덜어내나"라는 **상대 델타**뿐이다(Phase 5 +63%/−17%과 같은 상대 비교 서사).
...
- **세션 축(D1):** 주체(alice) 단위로 위험을 평가해 그 주체의 결정을 ALLOW→DENY로 무효화한다.
  같은 IP의 다른 주체나, 같은 주체의 위험이 식으면 개별적으로 가역된다.
- **에지 축(D3):** IP 단위로 패킷을 버린다. 주체를 구분하지 않는다 — 그 IP에서 오는 모든 것이 끊긴다.
...
- **노드 로컬.** deny map은 이 게이트웨이 노드의 커널에만 있다. 다중 게이트웨이로 에지 차단을
  전파하려면 D1의 Redis fan-out처럼 별도 배포 채널이 필요하다(현재는 세션 축만 fan-out).
...
> ⚠️ 함정 메모: ①코드 변경 시 해당 컨테이너 이미지 반드시 재빌드(stale 이미지면 sourceIp 미전달로
> 주체 번역 실패). ②고아 에이전트/map 선점 주의 — 재실행 전 `pkill -f '/agent'` + XDP off로 map
> 회수. ③WSL idle-shutdown → 검증은 단일 세션 스크립트(헬스 대기 포함)로. (상세는
> [phase-d3-xdp.md](../phases/phase-d3-xdp.md) "함정/주의".)
