...
- **epoch 키-아웃** — PDP 결정에 주체별 위험 `epoch`를 실어 게이트웨이가 학습하고, epoch가 오르면 이전 캐시 결정을 즉시 무효화한다.
  적재 시 키는 학습된 최신 epoch가 아니라 그 결정이 운반한 epoch로 만든다. 위험 전이 순간 같은 주체에
  두 평가가 동시에 진행 중일 때, 뒤늦게 도착한 옛 세대의 stale ALLOW가 현재 세대의 DENY를 덮어쓰지 못하게
  하기 위해서다(옛 결정은 옛 세대 키에 고립돼 더는 조회되지 않는다).
