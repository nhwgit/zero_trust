...
> `network-ip`(네트워크 축 — 게이트웨이 소켓 피어, 커널이 패킷에서 보는 것과 같은 좌표)는
...
- **관측 하강:** 커널 XDP가 per-source-IP로 SYN을 세고, 사이드카가 임계 초과를 PIP에
  ...
  PIP가 그 IP를 **네트워크 축**(게이트웨이 소켓 피어 — 커널과 같은 좌표계,
...
- **커널 관측 지점이 게이트웨이 호스트** — LB/프록시 뒤 배치에선 커널이 보는 소스가
  전 클라이언트 공통의 LB IP라, 좌표계를 분리해도([§4.1](#41-설명-가능한-동적-위험점수-pip))
  L4 신호의 판별력 자체가 낮아진다(세션 축 재평가는 가중합이 오폭을 흡수하지만, IP 단위
  에지 차단은 그 LB 경유 전원을 끊는다). 확장은 두 갈래 — 신뢰 프록시/LB 대역에는 에지
  차단 지시를 생략하는 게이트(세션 축 재평가는 유지), 근본은 관측을 진짜 에지(LB 앞)로
  옮기고 PROXY protocol로 네트워크 축 좌표를 공급하는 것이다.
