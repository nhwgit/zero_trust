...
**요청 흐름:** 로그인(Keycloak) → JWT 발급 → Gateway가 매 요청마다 JWT 검증 →
PDP에 "이 사용자가 이 리소스에 접근 가능?" 질의 → PDP가 PIP에서 **주체 속성 + 동적 위험점수**를 받아 정책 평가 →
허용이면 `resource-api`로 통과(내부 신뢰헤더/ mTLS로 우회 직접호출 차단), 거부면 403.
...
- **부하검증 (k6):** 결정 캐시 도입 전후 비교 — **처리량 +45%, p99 −27%**. "캐시로 성능을 얻되 신선도를 위해 일부 반납"하는 트레이드오프를 의식적으로 다룸.
