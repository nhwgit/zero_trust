#!/bin/bash
# Step 4 (선택) — 상대 성능: "유저스페이스 처리 vs XDP 커널 드랍"의 게이트웨이 CPU before/after.
#
...
#    deny 상태로 고정하려고 deny map에 직접 핀한다 — 캐싱 부하테스트의 "게이트웨이만 토글" 통제와 같은 결.)
#
...
#   얼마나 덜어내나"라는 상대 델타뿐이다(캐싱 부하테스트의 +63%/−17%과 동일한 상대 비교 서사).
#
